...
def extra_partition(img):
    ...
    # cắt ảnh
    x1, y1, x2, y2 = sbd
    img_sbd = image[y1 + h_min:y2, x1 + int(w_min * 1.4):x2 - int(w_min * 1.8)]
    x1, y1, x2, y2 = ma_de
    img_ma_de = image[y1 + int(h_min * 1.24):y2 - int(h_max_average * 0.18),
                x1 + int(w_min * 1.4):x2 - int(w_max_average * 1.3)]
